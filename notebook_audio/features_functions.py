...
def compute_features(sig_t, sig_s, sig_c):
    ...
    t = np.linspace(0, (sig_t.shape[0] - 1)*(1/44100), sig_t.shape[0])
    ...
    t_c = np.linspace(0, (sig_c.shape[0] - 1)*(1/44100), sig_c.shape[0])
    ...
    for feat_t in feat_temp:
        if feature_functions[feat_t[0]](sig_t, feat_t[1]) == inf or feature_functions[feat_t[0]](sig_t, feat_t[1]) == - inf:
            print(feature_functions[feat_t[0]])
        features.append(feature_functions[feat_t[0]](sig_t, feat_t[1]))
        
    for feat_s in feat_spec:
        
        if feature_functions[feat_s[0]](sig_s, feat_s[1]) == inf or feature_functions[feat_s[0]](sig_s, feat_s[1]) == - inf:
            print(feature_functions[feat_s[0]])
        features.append(feature_functions[feat_s[0]](sig_s, feat_s[1]))
        
    for feat_c in feat_ceps:
        
        if feature_functions[feat_c[0]](sig_c, feat_c[1]) == inf or feature_functions[feat_t[0]](sig_t, feat_t[1]) == - inf:
            print(feature_functions[feat_t[0]])
...
def RMS_sig(signal, args):
    ...
    return np.sqrt(np.abs(1/E*((args[0]**2)*sig_sq).sum() - cent**2))
